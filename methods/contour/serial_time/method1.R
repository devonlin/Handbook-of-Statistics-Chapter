...
#------------------------------------------------------    
   ...
   if(length(A_min)==0|| length(A_2)==0){
     ...
     ratio_Amin=matrix(c((sqrt(x_new_Amin[,(p+q+2)])/max(abs(x_new_Amin[,(p+q+1)]-a),epsilon)),1),nrow=1)
     
     #A_2
     ECL_A2 = ECL_f(XZ_pred_A2[,1:(p+q)],model,a)
     x_new_A2 = matrix(c(XZ_pred_A2[ECL_A2$ECL_id,],ECL_A2$ypred_max,ECL_A2$mse_max),nrow=1)
     ratio_A2=matrix(c((sqrt(x_new_A2[,(p+q+2)])/max(abs(x_new_A2[,(p+q+1)]-a),epsilon)),2),nrow=1)
